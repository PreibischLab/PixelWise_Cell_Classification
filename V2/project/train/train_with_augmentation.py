...
from lib.train.data_generator import batch__aug_data_generator

from params import *
from datetime import date
from tensorflow.keras.callbacks import ModelCheckpoint
from lib.train.callbacks import PlotLosses,SaveHistory
from lib.train.augmentation import getImgAug
...
base_name = d1+'augmentation'

model_file = os.path.join(OUTPUT_PATH,base_name+'.hdf5')
print(model_file)

augmentation = getImgAug()

gen_train_fast = batch__aug_data_generator(ZARR_FOLDER_TRAIN, batch_size=gp_batch_size
                                              ,voxel_shape = gp_voxel_shape,
                                              input_shape= gp_input_shape,
                                              output_shape = gp_output_shape,without_background=True,validate = True,seq= augmentation )

gen_test_fast = batch__aug_data_generator(ZARR_FOLDER_TEST, batch_size=gp_batch_size
                                              ,voxel_shape = gp_voxel_shape,
                                              input_shape= gp_input_shape,
...
unet_model.compile(optimizer = Adam(lr = 1e-3), loss = cross_entropy , metrics=['accuracy', MeanIoU( num_classes=unet_output_size)])
